...
| controller | `controllers/*` | what is per route: which service, which repository, which messages (the shared id/timing/log/save block is `attempt.ts`) | build SQL, call OpenAI directly |
...
`http.ts` is the one thing every layer shares: `fail()`, `waitUntil()`, the `AppEnv` context type.
`controllers/attempt.ts` is shared by the provider routes only.
...
- **Saving never fails the request.** `waitUntil(...)` plus `.catch((dbError) => console.error(...))`
...
- **Validation happens once, at the boundary.** Controllers read `c.get('body') as …` and trust
  ...
  downstream.
- **A provider route's body is `startAttempt(tag).run(...)`** (`controllers/attempt.ts`): it owns
  the request id, the timing, the `→ ← ✗` lines, the `waitUntil` save on both outcomes and the 502.
  A controller supplies only what is per route — request, service, repository, failure message —
  and uses `attempt.error(...)` for any side task it fires itself, so that failure carries the same
  id. Do not hand-roll a second copy of that block; extend `attempt` instead.

## Where new code goes

| Adding… | Touch |
| --- | --- |
| a route | `controllers/<name>.ts` + one line in `app.ts`; a provider route is a `startAttempt(tag).run(...)` call |
