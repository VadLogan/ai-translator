...
- `npm run dev:api` (root): `supabase functions serve --env-file supabase/functions/.env` (no name — it serves both `api` and `health`), on http://127.0.0.1:54321/functions/v1/. Deno runs the TS directly — no build step, so keep API code erasable: no enums, no parameter properties.
...
**Runtime flow**: The content script and the options page talk to the background worker only through the typed protocol in `src/messaging/messages.ts` (`translate`, `open-options`, `sign-in`, `sign-out`, `get-account`). Sign-in must run in the worker: `chrome.identity` is unavailable to content scripts and the options page. Responses are `{ ok, data } | { ok: false, error }`, because `Error` objects don't survive messaging. When adding a message type, update both `Message` and `ResponseMap`. The API call must stay in the background worker: `host_permissions` exempt it from page CORS, and on HTTPS pages a content-script fetch to an `http://` API would be blocked as mixed content.
